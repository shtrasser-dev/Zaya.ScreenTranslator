...
/// </summary>
public static class CaptureRegionsSnapshotService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var capture = EngineFactory.CreateCapture(engineId);
        if (capture is null)
            return null;
        ...
        using var session = await capture.CreateSessionAsync(
            region,
            ManagedSettingKeys.PrepareForEngine(capture.EngineId, capture.Settings, captureSettings),
    ...
    /// </summary>
    internal static bool IsUsableFrameSize(IRawImage frame, nint windowHandle)
    {
    ...
    internal static bool IsFullyBlack(IRawImage image)
    {
        var w = image.Width;
        var h = image.Height;
        if (w <= 0 || h <= 0)
